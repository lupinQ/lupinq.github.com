...
int main(int argc, char *argv[]){
	...
	infd = open("1.txt", O_RDWR| O_CREAT, 0600);
	if(infd == -1){
		perror("Open failed");
		ret = -1;
		goto fail;
	...
	infd1 = open("2.txt", O_RDWR| O_CREAT, 0600);
	if(infd1 == -1){
		perror("Open failed");
		ret = -1;
		goto fail;
	...
	outfd = open("3.txt", O_RDWR| O_CREAT, 0600);
	if(outfd == -1){
		perror("Open failed");
		ret = -1;
		goto close_fail3;
...
/*****************************************************************************************/
	...
	to = mmap(0, (size_t)infstat1.st_size, PROT_WRITE, MAP_SHARED, outfd, 0);
	...
	memcpy(to+(size_t)infstat.st_size,from1,infstat1.st_size);
/***************************************************************************************/
	if( -1 == msync(to, (size_t)infstat1.st_size, MS_SYNC)){
		perror("msync failed");
		ret = -1;
	}
